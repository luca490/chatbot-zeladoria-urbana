...
export const TicketModal: React.FC<TicketModalProps> = ({
  ...
  // Divide o relato original e as replicações adicionais com base nas tags de IA
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-[8px] cursor-pointer"
      onClick={onClose}
    >
      <motion.div
      ...
      >
        <div className="p-6 border-b border-[var(--color-border)] flex items-center justify-between sticky top-0 bg-[var(--color-surface)] rounded-t-2xl z-10">
          <div>
            <h2 className="text-2xl font-display font-bold text-[var(--color-foreground)] flex items-center gap-2">
          ...
          <button 
            onClick={onClose}
            className="p-2 bg-black/10 dark:bg-black/20 hover:bg-[var(--color-border)] rounded-full transition-colors text-[var(--color-muted)] hover:text-[var(--color-foreground)]"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-6 overflow-y-auto space-y-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-black/5 dark:bg-black/20 rounded-xl p-4 border border-[var(--color-border)]">
              ...
              <p className="text-[var(--color-foreground)] text-sm">{new Date(ticket.created_at).toLocaleString('pt-BR')}</p>
            </div>
            <div className="bg-black/5 dark:bg-black/20 rounded-xl p-4 border border-[var(--color-border)]">
            ...
            <div className="bg-black/5 dark:bg-black/20 rounded-xl p-4 border border-[var(--color-border)]">
              ...
              <p className="text-[var(--color-foreground)] font-bold mt-1 text-lg">{ticket.report_count || 1}</p>
            </div>
          </div>

          <div>
            <h3 className="text-lg font-bold text-[var(--color-foreground)] mb-2 border-b border-[var(--color-border)] pb-2">Relatos e Histórico</h3>
            <div className="space-y-4">
              {aiSummary && (
                <div className="bg-purple-500/10 border border-purple-500/30 rounded-xl p-5 mb-2 shadow-[0_0_15px_rgba(168,85,247,0.15)] relative overflow-hidden">
                  ...
                  <h4 className="text-purple-600 dark:text-purple-400 font-bold mb-2 flex items-center gap-2">
                    <span className="text-lg">✨</span> Resumo por IA
                  </h4>
                  <p className="text-purple-900 dark:text-purple-100/90 leading-relaxed font-medium relative z-10">{aiSummary}</p>
                </div>
              )}

              <div className="space-y-3">
                {reportsToShow.map((report, idx) => (
                  <div key={idx} className="bg-[var(--color-border)]/15 rounded-xl p-5 border border-[var(--color-border)]/40">
                    ...
                    <p className="text-[var(--color-foreground)]/90 leading-relaxed whitespace-pre-wrap font-medium">{report}</p>
                  </div>
                ))}
                ...
                {showAllReports && allReports.length > 3 && (
                  <button 
                    ...
                    className="text-xs text-[var(--color-muted)] font-semibold mt-3 hover:bg-[var(--color-border)]/20 transition-colors rounded-xl w-full text-center py-3 border border-[var(--color-border)] cursor-pointer flex items-center justify-center gap-1"
                  >
                    Ocultar relatos
                  </button>
                )}
          ...
          {ticket.image_url && (
            <div>
              <h3 className="text-lg font-bold text-[var(--color-foreground)] mb-2">Anexo</h3>
          ...
          <div>
            <h3 className="text-lg font-bold text-[var(--color-foreground)] mb-2 border-b border-[var(--color-border)] pb-2">Dados de Contato</h3>
            <div className="flex gap-4">
              <div className="bg-black/5 dark:bg-black/20 rounded-xl p-4 border border-[var(--color-border)] flex-1">
                <p className="text-xs text-[var(--color-muted)] uppercase tracking-wider mb-1 font-semibold">Nome</p>
                <p className="text-[var(--color-foreground)]">{ticket.user_name}</p>
              </div>
              <div className="bg-black/5 dark:bg-black/20 rounded-xl p-4 border border-[var(--color-border)] flex-1">
                <p className="text-xs text-[var(--color-muted)] uppercase tracking-wider mb-1 font-semibold">Telefone / WhatsApp</p>
                <p className="text-[var(--color-foreground)]">{ticket.user_phone}</p>
              </div>
            </div>
          </div>
        </div>

        <div className="p-6 border-t border-[var(--color-border)] bg-black/5 dark:bg-black/20 rounded-b-2xl">
          <label className="block text-sm font-semibold text-[var(--color-foreground)] mb-2">Gestão do Chamado</label>
          <div className="flex gap-4">
            <select
              value={editStatus}
              onChange={(e) => setEditStatus(e.target.value)}
              className="flex-1 bg-[var(--color-surface)] border border-[var(--color-border)] text-[var(--color-foreground)] rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-[var(--color-accent)] cursor-pointer font-medium"
            >
